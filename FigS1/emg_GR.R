...
# load data
data_all = read.csv("./bursts_BFp_SM_GRc.csv")

####################################################################################

# desired muscle
desMuscName <- "GRc"
desMusc     <- as.factor(desMuscName);
...
# extract data
data_red <- data_all[data_all$ratID %in% desRatID & data_all$muscle %in% desMusc & data_all$des_days %in% desDays,]

data_red$ratID    <- factor(data_red$ratID)
data_red$muscle   <- factor(data_red$muscle)
data_red$des_days <- as.factor(data_red$des_days)

# Data transformation
data_red_trf      <- data_red
data_red_trf$data <- log(data_red_trf$data)
...
pred <- expand.grid(muscle=unique(data_red$muscle),des_days=unique(data_red$des_days))

# polulation level (level=0) predictions
y=predict(lev.lm, level=0, newdata=pred)
pred$pred_val = y

pred <- pred[order(pred$muscle, pred$des_days),]

############ Compute menas ############
# remove non-used columns and NaN
dat_lev.small <- data_red_trf[,c("ratID","muscle","des_days","data")]
dat_lev.small <- dat_lev.small[!is.na(dat_lev.small$data),]

# mean over gait cycle and rats
data_red.mean <- aggregate(dat_lev.small$data, by = list(dat_lev.small$muscle, dat_lev.small$des_days), FUN = 'mean')
colnames(data_red.mean) <- c("muscle","des_days","data_mean")

data_red.mean <- data_red.mean[order(data_red.mean$muscle, data_red.mean$des_days),]
...
data_red.meanSbj <- aggregate(dat_lev.small$data, by = list(dat_lev.small$ratID, dat_lev.small$muscle, dat_lev.small$des_days), FUN = 'mean')
colnames(data_red.meanSbj) <- c("ratID","muscle","des_days","data_mean")

# Mean of the means
data_red.meanSbjAll <- aggregate(data_red.meanSbj$data_mean, by = list(data_red.meanSbj$muscle, data_red.meanSbj$des_days), FUN = 'mean')
colnames(data_red.meanSbjAll) <- c("muscle","des_days","data_mean")
...
# unique values of the factors
pred.sbj <- expand.grid(ratID=unique(data_red$ratID), muscle=unique(data_red$muscle), des_days=unique(data_red$des_days))

# polulation level (level=0) predictions
y=predict(lev.lm, level=1, newdata=pred.sbj)
pred.sbj$pred_val = y

pred.sbj <- pred.sbj[order(pred.sbj$ratID, pred.sbj$muscle, pred.sbj$des_days),]
...
data_red.mean <- aggregate(dat_lev.small$data, by = list(dat_lev.small$ratID, dat_lev.small$muscle, dat_lev.small$des_days), FUN = 'mean')
colnames(data_red.mean) <- c("ratID","muscle","des_days","data_mean")

data_red.mean <- data_red.mean[order(data_red.mean$ratID, data_red.mean$muscle, data_red.mean$des_days),]
